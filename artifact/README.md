...
- `klee`, `cbmc`, `scuba`: use the non-Datalog reference implementations described in Section 6.1 of the paper
...
This should output the derived facts (i.e., the IDB), like this:

```
Parsing...
Finished parsing (0.111s)
Type checking...
Finished type checking (0.009s)
Rewriting and validating...
Finished rewriting and validating (0.183s)
Evaluating...
Finished evaluating (0.053s)
...
To interpret the program with eager evaluation, add the `--eager-eval` flag:

```bash
java -jar lib/formulog.jar formulog/examples/greeting.flg --dump-idb --eager-eval
```

You should see an output similar to the prior run.

To compile the program, run Formulog with the `--codegen` flag:
...
If you are curious, the C++ file (generated by Soufflé) implementing semi-naive or eager evaluation of the program is `codegen/build/formulog.cpp`; you might want to format it first before checking it out:
